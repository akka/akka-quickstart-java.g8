...
This guide introduces Akka by describing the Java version of the Hello World example. If you prefer to use Akka with Scala, switch to the [Akka Quickstart with Scala guide](http://developer.lightbend.com/start/?group=akka&project=akka-scala-seed). 
...
1. Download the zip file from [Lightbend Tech Hub](http://dev.lightbend.com/start/?group=akka&project=akka-java-seed) by clicking `CREATE A PROJECT FOR ME`. 
